<?php
    ...
    class SphericalGeometry
    {
     ...
     public function distance($PointA, $PointB)
     {
          ...
          $c = $PointB->getLat() * $ra;;
          $d = $b - $c;
          $g = $PointA->getLng() * $ra - $PointB->getLng() * $ra;
          $f = 2 * asin(sqrt (pow(sin($d/2), 2) + cos($b) * cos($c) * pow(sin($g/2), 2)));
          $total=  $f * 6378.137;
          return $total;
     }
        ...
        protected static function _computeDistanceInRadiansBetween($LatLng1, $LatLng2)
        {
         ...
         $result =   asin(sqrt(pow(sin(($p1RadLat - $p2RadLat) / 2), 2) + cos($p1RadLat) 
